...
function getBehavioralInsight(expenses) {
  ...
  const catTotal = (mk, cat) => expenses.filter(e => e.category === cat && e.date?.startsWith(mk)).reduce((s, e) => s + e.amount, 0);
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }} className="summary-grid">
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', top: -40, right: -40, width: 160, height: 160, borderRadius: '50%', background: 'radial-gradient(circle,rgba(34,197,94,.15) 0%,transparent 70%)', pointerEvents: 'none' }} />
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          <div style={{ width: 28, height: 28, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(34,197,94,.2)', boxShadow: '0 0 12px rgba(34,197,94,.3)' }}>
            <Sparkles size={14} color="var(--green)" />
          </div>
          <span style={{ fontSize: '.65rem', fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase', color: 'var(--green)' }}>
            AI Smart Insight
          </span>
        </div>
        <p style={{ fontSize: '.95rem', fontWeight: 500, lineHeight: 1.6, color: 'var(--t1)', marginBottom: 20 }}>
          {insight}
        </p>
        <button style={{
        ...
        <div style={{ marginTop: 24, paddingTop: 20, borderTop: '1px solid rgba(255,255,255,.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '.8rem', color: 'var(--t2)' }}>
              <Target size={14} color="var(--tm)" /> Monthly Budget
            </div>
            {hasBudget && (
              <span style={{ fontSize: '.75rem', fontWeight: 700, color: over ? 'var(--red)' : 'var(--green)' }}>
          ...
          <input
            ...
            style={{ background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.1)', borderRadius: 10, color: 'var(--t1)', fontSize: '.875rem', padding: '8px 14px', outline: 'none', maxWidth: 200 }}
          />
          {hasBudget && (
            <div style={{ marginTop: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '.72rem', color: 'var(--tm)', marginBottom: 4 }}>
                <span>{formatCurrency(spend)} spent</span><span>{Math.round(pct)}%</span>
              </div>
              <div style={{ height: 8, borderRadius: 99, background: 'rgba(255,255,255,.08)', overflow: 'hidden' }}>
                <div style={{
                  height: '100%', borderRadius: 99, width: `${pct}%`, transition: 'width .7s ease',
                  background: over ? 'linear-gradient(90deg,#ef4444,#dc2626)' : pct > 75 ? 'linear-gradient(90deg,#f59e0b,#d97706)' : 'linear-gradient(90deg,#22c55e,#16a34a)',
                }} />
              </div>
            </div>
          )}
      ...
      {/* Breakdown */}
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 20, padding: 24, boxShadow: 'var(--shadow-md)' }}>
            ...
            return (
              <div key={cat} style={{ marginBottom: 16 }}>
                ...
                <div style={{ height: 6, borderRadius: 99, background: 'rgba(255,255,255,.06)', overflow: 'hidden' }}>
                  <div style={{ height: '100%', borderRadius: 99, width: `${p}%`, background: color, boxShadow: `0 0 6px ${color}60`, transition: 'width .7s ease' }} />
                </div>
              </div>
            );
          })
