  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <label style={{
        ...
        fontSize: '.72rem', fontWeight: 700, color: 'var(--t2)',
...
function FocusInput({ hasError, ...props }) {
  ...
  return (
    <input
      ...
      style={{
        ...baseInput(hasError),
        ...(focused ? { borderColor: 'var(--green)', boxShadow: '0 0 0 3px var(--green-dim)' } : {}),
        ...(props.style ?? {}),
      }}
...
function FocusSelect({ hasError, children, ...props }) {
  const [focused, setFocused] = useState(false);
  return (
    <select
      {...props}
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }} className="form-grid">
            ...
            <Field label="Category" icon={Tag}>
              <FocusSelect value={form.category} onChange={set('category')} required>
            ...
            <Field label="Date" icon={Calendar} error={errors.date}>
              <FocusInput
                ...
                max={getToday()} hasError={!!errors.date}
              />
            </Field>
