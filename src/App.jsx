...
import { Plus, Zap, Lightbulb, ArrowRight } from 'lucide-react';
...
function ComingSoon({ view, onBack }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, gap: 16, padding: 60, textAlign: 'center' }}>
      <div style={{ width: 64, height: 64, borderRadius: 20, background: 'var(--green-dim)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 32px rgba(34,197,94,.2)' }}>
      ...
      <button onClick={onBack} style={{
        padding: '9px 18px', borderRadius: 12, border: '1px solid rgba(34,197,94,.2)',
        background: 'var(--green-dim)', color: 'var(--green)',
        fontWeight: 700, fontSize: '.825rem', cursor: 'pointer',
      }}>
        ← Back to Dashboard
      </button>
    </div>
...
export default function App() {
  ...
  const headingStyle = { fontWeight: 800, fontSize: '1.3rem', color: 'var(--t1)' };
  const subStyle     = { fontSize: '.8rem', color: 'var(--tm)', marginTop: 4 };
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--bg)' }}>
        ...
        <div className="main-content" style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden', paddingLeft: 'var(--sidebar-w)' }}>
          ...
          <main style={pageStyle}>
            ...
            {/* Expenses */}
            {activeView === 'expenses' && <>
              <div><h1 style={headingStyle}>Expenses</h1><p style={subStyle}>Manage, search, filter and export your expense records.</p></div>
            ...
            {/* Analytics */}
            {activeView === 'charts' && <>
              <div><h1 style={headingStyle}>Analytics</h1><p style={subStyle}>Visualize where your money is going.</p></div>
              <DashboardCards stats={stats} expenses={expenses} />
